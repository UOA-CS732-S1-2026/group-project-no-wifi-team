  ...
  return (
    ...
    >
      ...
      <section className="relative z-20 h-dvh w-full overflow-hidden px-4 py-3 sm:px-8 sm:py-4">
        ...
        />
        {auth.token && (
          <div className="absolute left-1/2 top-[78%] z-30 -translate-x-1/2 mt-5 flex flex-col items-center gap-2">
            <p className="text-sm font-bold text-[#5a3010]">Welcome, {auth.username || 'Player'}</p>
            <button
              type="button"
              onClick={handleLogout}
              className="rounded-full border border-[#6b3f25] bg-[#f7e8c6] px-5 py-1 text-xs font-bold text-[#7a4b2b] hover:bg-[#e8d5a8] transition"
            >
              Logout
            </button>
          </div>
